...
int run_console_command(const std::string& cmdline) {
    if (!s_system) return -1;
    auto args_vec = split_args(cmdline);
    if (args_vec.empty()) return -1;

    // Build argc/argv
    std::vector<char*> argv;
    argv.reserve(args_vec.size());
    for (auto& a : args_vec) argv.push_back(const_cast<char*>(a.c_str()));
    int argc = static_cast<int>(argv.size());

    const std::string& cmd = args_vec[0];
    if (cmd == "status") return cmd_status(argc, argv.data());
    if (cmd == "ticket") return cmd_ticket(argc, argv.data());
    if (cmd == "publish") return cmd_publish(argc, argv.data());
    if (cmd == "gpio") return cmd_gpio(argc, argv.data());
    if (cmd == "test") return cmd_test(argc, argv.data());
    if (cmd == "?" || cmd == "help") return cmd_help(argc, argv.data());
